(function () {
  ...
  function selectedQuestions() {
    ...
    return questions.filter(function (question) {
      var matchesSource = !source || question.source === source;
      var matchesYear = !year || (question.years || []).map(String).indexOf(year) !== -1;
      var matchesSet = !set || String(question.set || "") === set;
      var matchesSubject = !subject || question.subject === subject;
      var matchesChapter = !chapter || question.chapter === chapter;
      var matchesType = !type || question.type === type;
      var searchText = [question.title, question.stem, question.subject, question.chapter, question.source]
  ...
  function updateDependentFilters() {
    ...
    var sourceQuestions = questions.filter(function (question) { return !source || question.source === source; });
    var currentSubject = controls.subject.value;
    populateSelect(controls.subject, sourceQuestions.map(function (question) { return question.subject; }), currentSubject, "全部科目");
    var subject = controls.subject.value;
    var chapterQuestions = sourceQuestions.filter(function (question) { return !subject || question.subject === subject; });
    populateSelect(controls.chapter, chapterQuestions.map(function (question) { return question.chapter; }), controls.chapter.value, "全部章节");
  }
