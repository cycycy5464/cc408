...
[tag_link]
正确答案：D
[Cache](../../../constitution_principle/storage/cache/content.md) 中存放的是主存的一部分副本，[TLB](../../../constitution_principle/storage/virtual/content.md#tlb)（快表）中存放的是 Page（页表）的一部分副本。在同时具有虚拟页式存储器（有 TLB）和 Cache 的系统中，CPU 发出访存命令，先查找对应的 Cache 块。
