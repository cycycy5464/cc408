...
路由器 内部有一个叫 [**路由表**](</computer_network/network/dev/#%e8%b7%af%e7%94%b1%e8%a1%a8>) 的数据结构，里面记录了各种目的 IP 地址该怎么走。这个表告诉 路由器：当收到一个 IP Packet 时，应该把包发给哪个 **下一跳** （下一个路由器\)。
...
**BGP** （Border Gateway Protocol，边界网关协议）是互联网的核心路由协议，用于在不同 [**自治系统**](</computer_network/network/route/#%e8%87%aa%e6%b2%bb%e7%b3%bb%e7%bb%9f>) 之间交换路由信息，属于路径向量（Path Vector）协议，目前广泛使用的版本是 BGP-4。
