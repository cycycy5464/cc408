...
**PPO** 和 **VPO** 的内容一致，所以地址翻译主要在于通过将 **VPN** 转化为 **PPN** 。**VPN** 的值为对应的 **页表项** （PTE）在 **页表** 中的下标。找到对应的 **页表项** 后，判断 **Valid** 字段是否为 **1** ：
  ...
  * 若为 **0** ，会发生 [缺页中断](/docs/os/ch02-memory/virtual/#%e7%bc%ba%e9%a1%b5%e4%b8%ad%e6%96%ad)
...
CPU状态
  ...
  * 当操作系统进行进程 [上下文切换](/docs/os/ch01-process/scheduling/#%e4%b8%8a%e4%b8%8b%e6%96%87%e5%88%87%e6%8d%a2) 时，它会：
1. 保存当前进程的CPU状态到其PCB。
...
与 [cache 存储结构](/docs/computer-org/ch02-memory/cache/#cache-%e5%ad%98%e5%82%a8%e7%bb%93%e6%9e%84) 类似，**TLB** 由许多 **表项** （TLB Entry）构成，每一个 **表项** 包含多个字段，其中 **tag** 和 **PPN** 是必须的，其他字段是可选的。
