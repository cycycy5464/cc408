...
在介绍 CPU 的组成结构之前，首先回顾一下计算机的 [冯诺依曼结构](/docs/computer-org/ch02-memory/overview/#%e5%86%af%e8%af%ba%e4%be%9d%e6%9b%bc%e7%bb%93%e6%9e%84)，其中 **运算器** 、**控制器** 、**存储器** 、**输入** 、**输出** 为计算机组成的五大部件，其中 CPU 涵盖的功能就包含 **运算器** 和 **控制器** 这两个。
...
CPU 的 **数据通路** 是指 CPU 在执行指令的过程中内部数据流动的路径，**ALU** 、**寄存器组** 、**多路选择器** 都属于数据通路的一部分。数据通路 **描述了** 信息从哪里开始，中间经过哪些部件，最后被传送到哪里。数据通路由 [控制部件](/docs/computer-org/ch04-cpu/controller/) 控制，控制部件根据每条指令功能的不同，生成对数据通路的控制信号。
...
控制器的功能和组成详见 [该节](/docs/computer-org/ch04-cpu/controller/)。
...
上文已经提到了 [CPU 的组成结构](/docs/computer-org/ch04-cpu/cpu-index/#%e7%bb%84%e6%88%90%e7%bb%93%e6%9e%84) 可以分为 **数据通路** 和 **控制器** 这两部分，**8086** 中的 **数据通路** 由指令队列、[寄存器](/docs/computer-org/ch04-cpu/cpu-index/#%e5%af%84%e5%ad%98%e5%99%a8) 和内总线组成。**控制器** 中包含指令译码器、时序控制单元、标志控制逻辑和微指令控制逻辑。
...
指令在 **代码段** （.text 段）中是从低地址向高地址增长的，这种增长方式也符合 **程序计数器** （PC 或 IP）的增长逻辑：在 [指令执行的取指阶段](/docs/computer-org/ch04-cpu/controller/#%e6%8c%87%e4%bb%a4%e6%89%a7%e8%a1%8c%e7%9a%84%e6%8e%a7%e5%88%b6%e4%bf%a1%e5%8f%b7)，控制单元在完成取指后会控制 **IP = IP + 指令长度** ，这样 **IP** 就指向了 **下一条待执行指令的地址** 。
...
**标志寄存器** （**Flags** ）用于存储处理器在 **执行指令过程中产生的各种状态和条件** ，这些状态可以进一步被 [控制器](/docs/computer-org/ch04-cpu/controller/) 所用，控制后续指令执行的行为。
...
* 当 [有符号整数](/docs/computer-org/ch01-data-representation/integers/#%e6%9c%89%e7%ac%a6%e5%8f%b7%e6%95%b0) 运算的结果太大而无法适应目标寄存器时，**OF** 标志会设置为 1，表示发生了溢出。



  * **CF** \(Carry flag\)：进位标志。
* 当 [无符号整数](/docs/computer-org/ch01-data-representation/integers/#%e6%97%a0%e7%ac%a6%e5%8f%b7%e6%95%b0) 运算的结果超出了目标寄存器的位数，**CF** 标志被设置为 1，表示发生了进位。
...
条件标志通常用于控制 [条件跳转指令](/docs/computer-org/ch03-instruction/instruction-index/#jxx-%e6%9d%a1%e4%bb%b6%e8%b7%b3%e8%bd%ac%e6%8c%87%e4%bb%a4) 的执行。条件跳转指令会检查条件寄存器中的标志位，根据标志位的状态决定是否将 **程序计数器** （PC）修改为目标地址，从而实现程序流程的跳转。
...
不过我们通过对一些位进行组合来 [实现](/docs/computer-org/ch01-data-representation/circuits/#%e5%8a%a0%e6%b3%95%e5%99%a8) 上述的逻辑结果。
...
**控制标志** 并不会被通常的运算指令自动修改，而是通过专门的指令来进行设置或清除。对于以下字段，了解即可：
  ...
  * **IF** \(Interrupt flag\)
* 控制中断处理
* 当 **IF** 被设置为 1 时，CPU 允许中断请求。如果 **IF** 为 0，CPU 将禁止所有 [**可屏蔽中断**](/docs/computer-org/ch05-bus/bus/#%e5%8f%af%e5%b1%8f%e8%94%bd%e4%b8%ad%e6%96%ad)，无论是外部硬件中断还是软件中断。
...
引脚名| 方向| 说明  
---|---|---  
**NMI**|  输入| [不可屏蔽中断](/docs/computer-org/ch05-bus/bus/#%e4%b8%8d%e5%8f%af%e5%b1%8f%e8%94%bd%e4%b8%ad%e6%96%ad)，请求立即响应  
**INTR**|  输入| [可屏蔽中断](/docs/computer-org/ch05-bus/bus/#%e5%8f%af%e5%b1%8f%e8%94%bd%e4%b8%ad%e6%96%ad) 请求  
...
**HOLD**|  输入| 外设 [请求总线控制权](/docs/computer-org/ch05-bus/bus/#%e6%ad%a5%e9%aa%a4)  
