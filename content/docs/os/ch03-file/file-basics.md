...
因此，`open` 实际上是一次向内核“申请访问权限”的操作，它返回 **文件描述符** （file descriptor），**文件描述符** 会被存储到进程的 [文件描述符表](/docs/os/ch03-file/filesystem/#%e6%96%87%e4%bb%b6%e6%8f%8f%e8%bf%b0%e7%ac%a6%e8%a1%a8) 中，并将 [系统文件打开表](/docs/os/ch03-file/filesystem/#%e7%b3%bb%e7%bb%9f%e6%89%93%e5%bc%80%e6%96%87%e4%bb%b6%e8%a1%a8) 中的引用计数 +1。此外，如果文件是第一次被打开时，该文件的 [inode](/docs/os/ch03-file/filesystem/#%e6%96%87%e4%bb%b6%e5%85%83%e4%bf%a1%e6%81%af) 也会从外存中被加载到内存中。
...
在 **Unix** 系统中，`write` 系统调用并不会立即将数据写入磁盘，而是将其暂存在内核的 **页缓存** （page cache）中。只有在特定时机——如缓存空间不足、文件被关闭，或系统周期性同步——才会将缓存中的数据统一刷新到磁盘（思路与 **cache** 中的 [回写法](/docs/computer-org/ch02-memory/cache/#%e5%9b%9e%e5%86%99%e6%b3%95) 一致，需要使用到脏位）。
...
在实践中还有 **多级索引** ，其思路与 [多级页表](/docs/os/ch02-memory/virtual/#%e5%a4%9a%e7%ba%a7%e9%a1%b5%e8%a1%a8) 一致。一级索引块中存储的是二级索引块的盘块号，最后一级索引块中才存储有数据块的盘块号。
