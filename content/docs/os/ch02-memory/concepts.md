...
页式虚拟存储的细节都在 [组成原理章节](/docs/os/ch02-memory/virtual/)，对于本节，重点掌握 **动态分区分配算法** 以及 **几种内存管理方式** （页式、段式、段页式）的概念和特点。
...
#### 基础概念
  ...
  * [**虚拟地址** 和 **物理地址** 空间](/docs/os/ch02-memory/virtual/#%e8%99%9a%e6%8b%9f%e5%86%85%e5%ad%98%e5%92%8c%e7%89%a9%e7%90%86%e5%86%85%e5%ad%98)

虚拟地址（VA, Virtual Address）由指令中的 **地址字段** 给出，进程看到的都是虚拟地址。物理地址是内存单元在 实际内存硬件 中的真实位置。

  * [**地址翻译**](/docs/os/ch02-memory/virtual/#%e5%9c%b0%e5%9d%80%e7%bf%bb%e8%af%91%e6%9c%ba%e6%9e%84)
...
当进程申请内存时，操作系统会遍历 **空闲分区表** ，按照预定的 **[适应算法](/docs/os/ch02-memory/concepts/#%e9%80%82%e5%ba%94%e7%ae%97%e6%b3%95)** （如首次适应、最佳适应等），从中选择一块满足需求的空闲区域，并完成内存分配。
...
具体来说，堆内存管理器维护着一个空闲内存块的列表（或类似数据结构），当进程申请内存时，管理器会根据 [适应算法](/docs/os/ch02-memory/concepts/#%e9%80%82%e5%ba%94%e7%ae%97%e6%b3%95) 寻找足够大的空闲块，将其一部分分配给请求，剩余部分可能仍作为空闲块保留。当进程释放内存时，该内存块会被回收并合并相邻的空闲块，以减少碎片。
...
页式管理的细节详见 [组成原理中的对应章](/docs/os/ch02-memory/virtual/#%e8%af%b7%e6%b1%82%e9%a1%b5%e5%bc%8f%e7%ae%a1%e7%90%86)，这里仅给出基本的思想以与 **段式管理** 和 **段页式管理** 进行对比。
