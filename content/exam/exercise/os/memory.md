...
正确答案：A
解释：
- [Belady](../../../../operating_system/memory/management/content.md#belady) 异常是指在特定场景下，增加物理内存页框数反而导致缺页中断次数增多的反直觉现象
...
正确答案：B
解释：
- [交换分区](../../../../constitution_principle/storage/virtual/content.md#%e4%ba%a4%e6%8d%a2%e5%88%86%e5%8c%ba) 主要用于存储进程数据
...
正确答案：C
当 RAM 更多时，物理内存中映射的虚拟页数会增加，因此发生的
[页面错误](../../../../constitution_principle/storage/virtual/content.md#%e9%a1%b5%e9%9d%a2%e9%94%99%e8%af%af)
更少。
...
解析：
为了支持虚拟内存，需要
[内存管理单元](../../../../constitution_principle/storage/virtual/content.md#%e5%9c%b0%e5%9d%80%e7%bf%bb%e8%af%91%e6%9c%ba%e6%9e%84)
...
正确答案：B
[抖动](../../../../operating_system/memory/management/content.md#%e6%8a%96%e5%8a%a8) 发生在系统中的进程所需内存超过可用内存时。如果进程没有“足够”的页面，页面错误率会非常高。这会导致：
- CPU 利用率低
- 操作系统大部分时间用于磁盘交换
这种行为就叫做抖动现象。
...
7. 虚拟内存页数
虚拟内存页数=2x246​=246−x
8. 方程求解23x−6=246−x⇒3x−6=46−x⇒4x=52⇒x=13
9. 页面大小计算
213
字节 = 8 KB
...
P=1061​
  - 页面错误服务时间
T_page=10ms=10×106ns
  - 内存访问时间
T_mem=20ns
2. 有效访问时间公式：Teffective​=P×Tpage​+(1−P)×Tmem​
3. 代入计算：Teffective​=(1061​)×10×106+(1−1061​)×20=10+20×(1−1061​)≈10+20=30ns
4. 结论：
有效访问时间约为 30 ns。
...
参见
[此处](../../../../constitution_principle/storage/virtual/content.md#%e5%a4%9a%e7%ba%a7%e9%a1%b5%e8%a1%a8)
。
...
虚拟地址的 9 位用于访问二级页表条目，且二级页表条目的大小为 4 字节。因此，二级页表的大小为
(29)×4=211
字节。这意味着有
(236)/(211)
个可能的位置可以存储此页表。因此，二级页表需要 25 位来寻址它。
同样地，一级页表需要 25 位来寻址它。
...
- TLB 命中率 = 0.6
计算过程：
Teff​​=0.6×(10+80)+0.4×(10+2×80)=0.6×90+0.4×170=54+68=122ms​
结论
...
```
为什么是 22 和 31？
- 22 ns：当 TLB 未命中时需访问 TLB (1 ns)，通过两级页表获取物理地址（2 次主存访问 × 10 ns），再从缓存中读取数据 (1 ns)，总计
1+2×10+1=22
ns。
- 31 ns：当 TLB 未命中且缓存未命中时，需访问 TLB (1 ns)，通过两级页表获取物理地址（2 次主存访问 × 10 ns），再从主存读取数据 (10 ns)，总计
1+2×10+10=31
ns。
...
帧
页表大小由页表项数量与每项大小决定：
页表大小=220×14位≈220×16位=220×2B=2MB
关键点：页表项需记录帧号（14 位）及控制位（如有效/无效标志），实际存储需向上取整至 16 位（即 2 字节）。最终页表大小为
220×2B=2MB
。
...
平均时间=p⋅t1​+(1−p)⋅(t2​+t1​)
代入数值计算：
=0.9999⋅1+0.0001⋅(10,000+1)=0.9999+1.0001=2ms
...
正确答案：A
[最佳适应](../../../../operating_system/memory/concepts/content.md#%e6%9c%80%e4%bd%b3%e9%80%82%e5%ba%94) 算法会为新进程在足够大的分区中选择最小的块进行分配。因此内存块的分配顺序如下：
- 357 KB → 选择 400 KB 分区
- 210 KB → 选择 250 KB 分区
- 468 KB → 选择 500 KB 分区
- 491 KB → 选择 600 KB 分区
...
正确答案：A
参考 [线程](../../../../operating_system/process/process_thread/content.md#%e7%ba%bf%e7%a8%8b) 一节：
- 线程无法共享栈（用于维护函数调用）
- 因为其可能具有各自的函数调用序列
...
- 基本参数
  - 数据块大小 = 1024 字节
  - 单个块可存储地址数 = 128
- 各类型地址对应容量`直接地址：10 × 1024 B 单级间接：128 × 1024 B 双级间接：128² × 1024 B 三级间接：128³ × 1024 B`
- 总容量计算
```
...
字节
- 转换为兆字节（MB）：
220226×6​=6×26=6×64=384MB
因此，选项 A 是正确答案。

##### 44

考虑一个具有十个物理页框的计算机系统。该系统提供了一个访问序列（a₁, a₂, &mldr;, a₂₀, a₁, a₂, &mldr;, a₂₀），其中每个 aᵢ 为数字。最后进先出（LIFO）页面置换策略与最佳（Optimal）页面置换策略在缺页中断次数上的差值是（ ）。
...
- 最终公式代入有效访问时间=0.90×60ns+0.10×110ns=65ns
...
可以使用自由链表或位图来跟踪磁盘空闲空间。磁盘地址需要
d
位。对于一个包含 13 个块的磁盘，其中有
F
个是空闲的，请说明在什么条件下自由链表占用的空间比位图更少（ ）？
...
内存为空，可用帧数 4 个。
2. 页面引用序列分析
按顺序处理每个页面请求，记录内存状态及页面错误：步骤访问页面内存状态（当前帧）是否页面错误替换页面10[0]是-22[0, 2]是-34[0, 2, 4]是-45[0, 2, 4, 5]是-52[0, 2, 4, 5]否-64[0, 2, 4, 5]否-73[0, 2, 3, 5]是4（LRU）811[0, 2, 3, 11]是5（LRU）92[0, 2, 3, 11]否-1010[0, 2, 3, 10]是11（LRU）
3. 关键逻辑
  - 页面错误条件：目标页面不在内存中。
...
B
步骤 1：计算页面数量
页面数量=页面大小逻辑地址空间​=212232​=220
步骤 2：计算页表大小
- 页表项大小：32 位系统中每个页表项占 4 字节（
22
B）
页表大小=页面数量×页表项大小=220×22=222B=4MB
结论：选项 (C) 正确。
...
EAT=HTLB​×TTLB​+(1−HTLB​)[TTLB​+4×Tm​]+Tm​
其中：
- HTLB​
：TLB 命中率
- TTLB​
...
1. 代入已知参数：
  ...
  - TTLB​=20ns
  - Tm​=100ns
2. 代入公式计算：EAT​=(0.98×20)+0.02(20+4×100)+100=19.6+0.02(20+400)+100=19.6+8.4+100=128ns​
选项 (B) 正确。
...
2. TLB 未命中但无缺页：需两次主存访问（页表查询 + 数据访问，即 100ns + 100ns）
3. TLB 未命中且发生缺页（页表查询 100ns）：
  - 80% 情况：仅读取页面（5000ns + 100ns）
  - 20% 情况：先写回脏页（5000ns）再读取（5000ns + 100ns）
完整公式如下：
​=h×(T+M)+(1−h)[T+(1−p)×2M+p(M+(1−d)×D+d×2D+M)]​
数值代入计算
​=0.95×(20+100)+0.05[20+0.9×200+0.1(100+0.8×5000+0.2×10000+100)=155ns​
选项 A 正确。
...
- 最大文件大小计算`总块数 = 直接指针 + 单级间接 + 双级间接 + 三级间接 = 10 + 2⁸ + (2⁸ × 2⁸) + (2⁸ × 2⁸ × 2⁸) ≈ 2²⁴ 块`
- 最终文件大小
  - 每块大小 = 2¹⁰ 字节（1Kbyte）
  - 总大小 = 2²⁴ × 2¹⁰ = 2³⁴ 字节
