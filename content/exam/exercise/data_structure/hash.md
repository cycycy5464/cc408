...
使用哈希函数
h(k)=kmod10
和线性探测法，有多少种不同的关键字值插入顺序会导致下图所示的哈希表？（ ）
...
正确答案：C
- 约束条件：
  - 元素 `42`、`23` 和 `34` 必须在 `52` 和 `33` 之前插入。
  - 元素 `46` 必须在 `33` 之前插入。
- 计算方式：不同序列总数=3!×5=30
- 公式解析：
  - `3!` 表示 `42`、`23` 和 `34` 可以以任意顺序插入（共 6 种排列）。
  - `5` 表示 `46` 可以在满足约束的前提下插入到 5 个合法位置中。
...
1. 哈希函数将任意长度的消息转换为固定长度的代码。
2. 哈希函数将固定长度的消息转换为可变长度的代码。
3. 不同的消息可能通过哈希函数得到相同的哈希值。
...
每个键发生冲突的概率是
201​
（因为总共有 20 个存储位置，且每个键只能映射到一个位置）。设插入
x
个键后概率变为
21​
，则有：
...
O(1)
的查找操作。
- 二分查找需在有序数组中进行，时间复杂度为
O(logn)
- 线性查找需遍历数据，时间复杂度为
O(n)
- AVL 树查找基于平衡二叉树结构，时间复杂度为
O(logn)
...
正确答案：B
- 661 的哈希值：`661 % 13 = 11` → 插入位置 11
- 182 的哈希值：`182 % 13 = 0` → 插入位置 0
- 24 的哈希值：`24 % 13 = 11`（冲突）
  - 线性探测找到下一个可用位置 12
- 103 的哈希值：`103 % 13 = 12`（冲突）
  - 线性探测找到下一个可用位置 1
- 因此键值 103 最终插入位置为 1，对应选项 (B)

##### 11

使用以下密码函数对明文 ISRO 进行加密时，当密钥
k=7
时会生成什么密文？[假设 ‘A’ = 0, ‘B’ = 1, &mldr;, ‘Z’ = 25]。

Ck​(M)=(kM+13)mod26

正确答案：A
Ck(M) = (kM + 13) mod 26
此处，‘A’ = 0, ‘B’ = 1, &mldr;&mldr;.‘Z’ = 25
- I 的加密过程
  - 明文字符 I 对应数值 8
  - 代入公式：
7×8+13=69
...
3. 计算过程：
  - 第一步：
123456×0.618033=76189.882048
  - 第二步（取小数部分）：
76189.882048mod1=0.882048
  - 第三步（乘以表长）：
100×0.882048=88.2048
  - 第四步（向下取整）：
⌊88.2048⌋=88
4. 结论：
键值 `123456` 对应的哈希地址为 88，因此选项 (C) 正确。
...
定义为
h(key)=keymod7
，并使用线性探测法解决冲突。将键值 44、45、79、55、91、18、63 插入索引范围为 0 到 6 的哈希表中。键值 18 最终会存储在哪个位置？（ ）
...
- 18：
h(18)=18mod7=4
，位置 4 被占用 → 探测位置 5（空闲）→ 存入位置 5
- 63：
h(63)=63mod7=0
，位置 0 被占用 → 探测位置 1（空闲）→ 存入位置 1
最终哈希表状态：
| 索引 | 内容 |
| --- | --- |
| 0 | 91 |
...
- 双重哈希公式地址=(h1​(k)+i×h2​(k))mod(表的大小)
- 计算过程
  1. h1​(k)=90mod23=21
  2. h2​(k)=1+(90mod19)=15
  3. 代入公式：
(21+15)mod23=36mod23=13
- 结论
选项 (A) 正确。
...
正确答案：C
- 选项 (A) 中的序列不可能，因为包含键 `4 (= 0100)`，但最终哈希表中没有该键。
- 选项 (B) 中的序列不可能，因为键 `1 (=0001)` 和 `9 (=1001)` 在基于第三最低有效位的 0 侧解决冲突的方式与最终结果不符。
- 选项 (C) 是正确的序列，可得到给定的最终哈希表。
- 选项 (D) 中的序列不可能，因为在第三列中包含键 `6 (=0110)`、`10 (=1010)` 和 `14 (=1110)`，而这些键在最终哈希表中并未出现。
...
。当
k=123456
时，第一次和第二次探测在槽位上的差值是多少？（ ）

正确答案：C
计算步骤：
1. 基础哈希值 h₁(k):h1​(123456)=123456mod701=80
2. 辅助哈希值 h₂(k):h2​(123456)=1+(123456mod700)=1+256=257
3. 第一次探测（i=1）:h(123456,1)=(80+1×257)mod701=337
4. 第二次探测（i=2）:h(123456,2)=(80+2×257)mod701=594
5. 两次探测的差值:594−337=257
结论： 正确答案为选项 C。
...
计算过程：
1. h(1) = (7×1 + 3) mod 4 = 10 mod 4 = 2
2. h(3) = (7×3 + 3) mod 4 = 24 mod 4 = 0
3. h(8) = (7×8 + 3) mod 4 = 59 mod 4 = 3
4. h(10)= (7×10 + 3) mod 4 = 73 mod 4 = 1
哈希表构建：
| 索引 | 内容 |
| --- | --- |
| 0 | 3 |
...
- (C) 成立，因为：
  ...
  - 所有插入操作均符合线性探测规则。
- (D) 不成立，因为序列中 `33` 出现在 `23` 之前。
  - `33` 插入时会占据位置 3，导致 `23` 插入时需从位置 3 开始探测，最终插入位置 4 或 5，但实际表中 `23` 位于位置 4，与插入顺序不符。
...
个关键字、
m
个哈希表槽位，以及两个简单且均匀的哈希函数
h1​
和
h2​
。进一步假设我们的哈希方案对奇数关键字使用
h1​
，对偶数关键字使用
h2​
。那么每个槽位中关键字的期望数量是多少？（ ）
...
正确答案：B
索引映射
（也称为平凡哈希）是一种简单的哈希方法，其中数据直接映射到哈希表中的索引。
